...
- (UIImage*) imageNamed:(NSString*)str{
	CGFloat s = 1.0f;
	if([[UIScreen mainScreen] respondsToSelector:@selector(scale)]){
		s = [[UIScreen mainScreen] scale];
	}
	NSString *path = [NSString stringWithFormat:@"%@%@.png",str,s > 1 ? @"@2x":@""];
    NSLog(@"UIImage.path=====>%@",path);
	return [UIImage imageWithCGImage:[UIImage imageWithContentsOfFile:BUNDLE(path)].CGImage scale:s orientation:UIImageOrientationUp];
