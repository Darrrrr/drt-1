...
#pragma mark - CalendarMonthTiles
@interface CalendarMonthTiles : UIView {
...
+ (void) initialize{
    if (self == [CalendarMonthTiles class]){
        tileImage = [UIImage imageWithContentsOfFile:BUNDLE(@"calendar/Month Calendar Date Tile.png")];
    }
}
...
- (id) initWithMonth:(NSDate *)date marks:(NSArray *)markArray startDayOnSunday:(BOOL)sunday timeZone:(NSTimeZone *)timeZone{
	...
	NSArray *dates = [CalendarMonthTiles rangeOfDatesInMonthGrid:date startOnSunday:sunday timeZone:self.timeZone];
...
- (void) drawRect:(CGRect)rect {
	...
	if(today > 0){
		...
		r.origin.y -= 6;
		[[UIImage imageWithContentsOfFile:BUNDLE(@"calendar/Month Calendar Today Tile.png")] drawInRect:r];
	}
...
- (BOOL) selectDay:(NSInteger)day{
	...
	if(day == today){
		...
		self.selectedImageView.image = [UIImage imageWithContentsOfFile:BUNDLE(@"calendar/Month Calendar Today Selected Tile.png")];
		markWasOnToday = YES;
		
	}else if(markWasOnToday){
		self.dot.shadowOffset = CGSizeMake(0, -1);
		self.currentDay.shadowOffset = CGSizeMake(0, -1);
		NSString *path = BUNDLE(@"calendar/Month Calendar Date Tile Selected.png");
...
- (void) reactToTouch:(UITouch*)touch down:(BOOL)down{
	...
	/*
	 When a UIViewController allocated and pushViewController it in delegate.- (void)calendarMonthView:(CalendarMonthView *)monthView didSelectDate:(NSDate *)date.
...
- (UILabel *) dot{
	...
	_dot.shadowColor = [UIColor whiteColor];//darkGrayColor
...
+ (void) initialize{
    if (self == [CalendarMonthView class]){
		gradientColor = [UIColor colorWithPatternImage:[UIImage imageWithContentsOfFile:BUNDLE(@"calendar/color_gradient.png")]];
		grayGradientColor = [UIColor colorWithPatternImage:[UIImage imageWithContentsOfFile:BUNDLE(@"calendar/color_gradient_gray.png")]];
...
- (id) initWithSundayAsFirst:(BOOL)s timeZone:(NSTimeZone*)timeZone{
	...
	self.backgroundColor = [UIColor colorWithHex:0xaaaeb6];//5.8修改原始aaaeb6
	self.timeZone = timeZone;
	self.sunday = s;
...
- (UIView *) topBackground{
	...
	//月历信息配置
	GradientView *gradient = [[GradientView alloc] initWithFrame:CGRectMake(0, 0, CGRectGetWidth(self.frame), TOP_BAR_HEIGHT)];
	gradient.colors = @[[UIColor colorWithHex:0xf4f4f5],[UIColor colorWithHex:0xccccd1]];//0xf4f4f5, 0xccccd1
	...
	line.backgroundColor = [UIColor colorWithHex:0xaaaeb6];//0xaaaeb6
...
- (UIButton *) leftArrow{
	...
	[_leftArrow setImage:[UIImage imageNamed:@"calendar_left_arrow.png"] forState:0];
	return _leftArrow;
}
- (UIButton *) rightArrow{
	...
	[_rightArrow setImage:[UIImage imageNamed:@"calendar_right_arrow.png"] forState:0];
	return _rightArrow;
}
- (UIView *) tileBox{
...
- (UIView *) shadow{
	...
	GradientView *grad  = [[GradientView alloc] initWithFrame:CGRectMake(0, 0, 100, CGRectGetWidth(self.frame))];
...
- (UIView *) dropshadow{
	...
	GradientView *grad  = [[GradientView alloc] initWithFrame:CGRectMake(0, 0, CGRectGetWidth(self.frame), 10)];
...
- (void) reloadData{
	...
	NSArray *dates = [CalendarMonthTiles rangeOfDatesInMonthGrid:[self.currentTile monthDate] startOnSunday:self.sunday timeZone:self.timeZone];
	NSArray *ar = [self.dataSource calendarMonthView:self marksFromDate:dates[0] toDate:[dates lastObject]];
	
	CalendarMonthTiles *refresh = [[CalendarMonthTiles alloc] initWithMonth:[self.currentTile monthDate] marks:ar startDayOnSunday:self.sunday timeZone:self.timeZone];
