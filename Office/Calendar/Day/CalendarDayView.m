...
@property (nonatomic,strong) NowView *nowLineView;
...
- (void) _setupView{
	...
	self.nowLineView = [[NowView alloc] init];
	...
	for(NSInteger i=0;i<3;i++){
		...
		TimelineView *timelineView = [[TimelineView alloc] initWithFrame:sv.bounds];
	...
	for(NSInteger i=0;i<3;i++){
		...
		WeekdaysView *weekdayView = [[WeekdaysView alloc] initWithFrame:weekFrame];
...
- (void) scrollViewWasTapped:(UITapGestureRecognizer*)gesture{
	if(self.delegate && [self.delegate respondsToSelector:@selector(calendarDayTimelineView:eventViewWasSelected:)])
		[self.delegate calendarDayTimelineView:self eventViewWasSelected:(CalendarDayEventView*)gesture.view];
}
...
- (void) _scrollToTopEvent:(BOOL)animated{
	...
	TimelineView *timeline = [self _timelineAtIndex:1];
...
- (TimelineView*) _timelineWithScrollView:(UIScrollView*)sv{
	return (TimelineView*)[sv viewWithTag:5];
}
- (TimelineView*) _timelineAtIndex:(NSInteger)index{
	return (TimelineView*)[self.pages[index] viewWithTag:5];
}
...
- (void) _refreshDataWithPageAtIndex:(NSInteger)index{
	...
	TimelineView *timeline = [self _timelineAtIndex:index];
	...
	for (UIView* view in sv.subviews) {
		if ([view isKindOfClass:[CalendarDayEventView class]]){
	...
	[timeline.events sortUsingComparator:^NSComparisonResult(CalendarDayEventView *obj1, CalendarDayEventView *obj2){
...
- (void) _realignEventsAtIndex:(NSInteger)index{
	...
	for (CalendarDayEventView *event in timeline.events) {
		...
		for (NSInteger i = [sameTimeEvents count]-1; i >= 0; i--) {
			CalendarDayEventView *sameTimeEvent = sameTimeEvents[i];
...
- (void) _advanceWeekToIndex:(NSInteger)index animated:(BOOL)animated{
	...
	WeekdaysView *weekView = self.weekdayPages[pageToUpdate];
	DateLabel *currentLabel = weekView.weekdayLabels[self.indexOfCurrentDay];
...
- (void) _resetWeekdayOrderByMoving:(BOOL)forwards{
	...
	if(forwards){
		WeekdaysView *weekView = [self.weekdayPages firstObject];
		[self.weekdayPages addObject:weekView];
		[self.weekdayPages removeObjectAtIndex:0];
		
		
		
		WeekdaysView *currentWeekView = self.weekdayPages[1];
		DateLabel *currentPageLabel  = currentWeekView.weekdayLabels.lastObject;
		...
		mutedCom.day++;
		for(DateLabel *label in weekView.weekdayLabels){
		...
		WeekdaysView *weekView = [self.weekdayPages lastObject];
		...
		WeekdaysView *currentWeekView = self.weekdayPages[1];
		DateLabel *currentPageLabel  = currentWeekView.weekdayLabels.firstObject;
		
		NSDateComponents *mutedCom = [self.calendar components:NSCalendarUnitDay | NSCalendarUnitMonth | NSCalendarUnitYear | NSCalendarUnitEra fromDate:currentPageLabel.date];
		mutedCom.day --;
		for(DateLabel *label in weekView.weekdayLabels.reverseObjectEnumerator){
	...
	NSInteger i = 0;
	for(WeekdaysView *sv in self.weekdayPages){
		CGRect r = sv.frame;
...
- (void) _updateSelectedWeekdayAtIndex:(NSInteger)allLabelsIndex{
	...
	NSInteger i=0;
	for(DateLabel *label in allLabel){
...
- (void) didTapWeekdayLabel:(UITapGestureRecognizer*)sender{
	...
	DateLabel *label = allLabels[indexOfSelectedLabel];
...
- (NSArray*) _allDayLabels{
	...
	for(WeekdaysView *page in self.weekdayPages){
...
- (void) tintColorDidChange{
	...
	for(DateLabel *label in [self _allDayLabels])
...
- (UIView *) daysBackgroundView{
	...
	_daysBackgroundView.backgroundColor = [UIColor colorWithHex:0xf7f7f7];//0xf7f7f7
	...
	div.backgroundColor = [UIColor colorWithHex:0xd7d7d7];//5.8修改原始d7d7d7
...
- (CalendarDayEventView*) dequeueReusableEventView{
	...
	CalendarDayEventView *event = [self.eventGraveYard lastObject];
...
- (void) drawRect:(CGRect)rect {
	...
	UIColor *lineColor = [UIColor colorWithHex:0xd7d7d7];//5.8修改原始d7d7d7
...
- (id) initWithFrame:(CGRect)frame{
	...
	for(NSInteger i=0;i<7;i++){
		DateLabel *label = [[DateLabel alloc] initWithFrame:CGRectMake(8+(DAY_LABEL_WIDTH+9)*i, 16, DAY_LABEL_WIDTH, DAY_LABEL_WIDTH)];
		label.weekend = i % 6 == 0;
